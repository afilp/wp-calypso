...
const PlanStatus = React.createClass( {
	...
	renderNudge() {
		const { rawDiscount: hasDiscount } = this.props.plan,
			day = getDaysUntilUserFacingExpiry( this.props.plan );

		if ( hasDiscount && ( day < 6 && day > 0 ) ) {
			const prices = this.getPrices();

			return (
				<div className="plan-status__nudge">
					{ this.translate(
						'{{del}}%(originalPrice)s{{/del}} {{strong}}%(newPrice)s{{/strong}} - {{em}}Save when you combine your domain and plan{{/em}}',
						{
							args: {
